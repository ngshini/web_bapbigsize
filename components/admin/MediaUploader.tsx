...
export function MediaUploader({ media, setMedia }: { media: Media[]; setMedia: (media: Media[]) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {hasPendingChanges ? (
        <p className="rounded-md bg-gold/30 p-2 text-sm font-semibold text-brand-900">
          Ảnh đã upload xong. Bấm “Lưu sản phẩm” ở cuối form để cập nhật lên website.
        </p>
      ) : null}
      <div className="grid gap-2">
        {media.map((item, index) => (
          <div
            ...
            className="flex flex-col gap-3 rounded-md border p-2 text-sm sm:flex-row sm:items-center sm:justify-between"
          >
            <div className="flex min-w-0 items-center gap-3">
              <div className="relative grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-md bg-brand-50 text-xs font-bold text-brand-700">
                {item.mediaType === "IMAGE" ? (
                  <Image
                    src={item.mediaUrl}
                    alt={item.altText ?? item.originalFileName ?? "Ảnh sản phẩm"}
                    fill
                    sizes="64px"
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                <option value="Xám nhạt">Xám nhạt</option>
              ...
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1">
                  <input
                    ...
                    onChange={() => {
                      setMedia(
                        media.map((current, currentIndex) => ({
                          ...current,
        ...
        ))} {/* <-- Đã sửa cấu trúc đóng ngoặc chuẩn ở đây */}
      </div>
    </div>
  );
}